...
function New-ProfilePackage {
    ...
    $readme='R4OS '+$Version+' ('+$Definition.Name+'). disk.img is 12 GB and contains BIOSBOOT/BOOT/SYSTEM/RECOVERY/DATA, with 10 GB SYSTEM. DATA is fresh. Update Recovery and restart it before updating from an older R4OS release. SYSTEM updates preserve the existing partition size; only a new installation creates 10 GB SYSTEM. A new installation erases the selected disk. Use a USB device of at least 16 GB. CreateUSB.bat / sh CreateUSB.sh uses PowerShell 7 and places the original release ZIP on the USB Recovery partition. Keep the ZIP and starter on another disk.'
